...
function initializeMermaidEditor() {
  ...
  if (typeof mermaid === 'undefined') {
    ...
    script.onload = () => {
      ...
      // Initialiser Mermaid avec une configuration pour désactiver les notifications d'erreur
      mermaid.initialize({
        startOnLoad: false,
        theme: 'default',
        logLevel: 'error', // Réduire les logs inutiles
        securityLevel: 'loose', // Permettre un rendu plus flexible
        suppressErrors: true // Désactiver les notifications d'erreur par défaut (les "bombes")
      });
      renderMermaid();
  ...
  function renderMermaid() {
    ...
    // Nettoyer les anciennes notifications d'erreur et le contenu de #mermaid-output
    mermaidOutput.innerHTML = '';
    // Supprimer les notifications "bombes" de Mermaid (si elles existent encore)
    const errorNotifications = document.querySelectorAll('.mermaid-error-notification');
    ...
    try {
      ...
      mermaid.render(uniqueId, code).then(result => {
        ...
        // Afficher l'erreur dans #mermaid-output avec un style personnalisé
        mermaidOutput.innerHTML = `<div class="mermaid-error"><p style="color: red;">${translations[currentLang]['mermaid_render_error'] || 'Erreur de rendu Mermaid'} : ${error.message}</p><p style="color: gray;">Vérifiez votre syntaxe. Consultez l’aide pour des exemples corrects.</p></div>`;
      });
    } catch (error) {
